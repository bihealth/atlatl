...
def get_parser():
    ...
    parser_breakends.add_argument("--min_alignments", type=int, default=0, help="minimum number of alignments for a single breakend to be considered and reported. min_alignments <= 0 means only significant breakends are considered and reported.")
    ...
    parser_assemble_and_visualize.add_argument("--bamout_reversed", type=os.path.abspath,required=True , help="output: same as [bamout] but for the reverse complement sequence.")
    ...
    parser_group_assemble_and_visualize.add_argument("--outdir", type=os.path.abspath,required=True , help="output: directory to where all assembly related data are saved.")
    parser_group_assemble_and_visualize.add_argument("--vizdir", type=os.path.abspath,required=True , help="output: directory to where all visualizations are saved.")
    ...
    workflow_prep_ont.add_argument("-f","--fastas", type=os.path.abspath,required=True , nargs='+', help="list of fasta-files that contain the sequences from which alleles defined in 'alleles' are constructed.")
    ...
    # =========================================================================
    #  WORKFLOW workflow_map_ont
    # =========================================================================
    workflow_map_ont = subparsers.add_parser("workflow_map_ont",description="Creates a configfile and runs snakemake map reads to the augmented reference and produces feature counts for the given features and repeats. Run this after workflow_prep_ont.")
    workflow_map_ont.add_argument("-w","--workdir",     type=os.path.abspath,   required=True, help="workdir of snakemake.")
    workflow_map_ont.add_argument("-a","--constructs",  type=os.path.abspath,   required=True, help="fasta-file of constructed sequences of the final alleles.")
    workflow_map_ont.add_argument("-i","--in_reads",    type=os.path.abspath,   required=True, help="fastq-file of reads.")
    workflow_map_ont.add_argument("-r","--reference",   type=os.path.abspath,   required=True, help="path to augmented reference.")
    workflow_map_ont.add_argument("-n","--annotations", type=os.path.abspath,   required=True, help="path to generated bed-file with annotations for the augmented reference.")
    workflow_map_ont.add_argument("-c","--cores",       type=int,   default=30,   help="cores argument passed to snakemake.")
    workflow_map_ont.add_argument("-d","--depth",       type=int,   default=0,    help="depth of recursion in read selection. Only useful if great re-arrangements of the knock-in are expected. Default is 0.")
    workflow_map_ont.add_argument("--minimap_r",        type=int,   default=95,   help="-r parameter of minimap2. Deletions greater than this value are detected in breakend-analysis. Smaller deletions remain within single alignments. Default is 95. This is feasible for read error-rates of less than 5 percent. Use greater values for higher error-rates.")
    workflow_map_ont.add_argument("--minimap_z",        type=int,   default=95,   help="-z parameter of minimap2. Inversions greater than this values can be detected. Default is 95. This is feasible for read error-rates of less than 5 percent. Use greater values for higher error-rates.")
    workflow_map_ont.set_defaults(func=workflows.workflow_map_ont)

#    workflow_map_ont.add_argument("-p","--in_repeats", type=os.path.abspath,required=True , help="bed-file of reads for given reference.")
